...
parser.add_argument('--min_learning_rate',         type=float,   help="min lr on reduce",           default = 0.003)
parser.add_argument('--split_val',                 type=float,   help='Validation set %%%',         default = 0.50)
...
parser.add_argument('--path_to_connectome',        type=str,     help='path_to_connectome',         default = './celegans.graphml')
...
parser.add_argument('--save_to',                   type=str,     help='save_to',                    default = './ElegansM2_echo')
...
class ElegansMnist280Recon:
    ...
    def init_graph_allocations(self, x, sensors, interneurons, motors,sensors_to, motors_to, interneurons_to, patch_pattern, activation_line):
                            ...
                            if self.gg.vs()['role'][coord_1] in ["M", 1] :
                                if isinstance(motors[coord_1], keras.engine.sequential.Sequential):
                                    ...
                                    if apply_cross_conn:
                                        ...
                                        rel_s = str(self.gg.vs()['role'][coord_0]) + str(self.gg.vs()['role'][coord_1])
                                        #print("Trovato" + rel_s)
                                        motors[coord_1] = tf.keras.layers.Multiply(name=self.t_n(rel_s, gg_el["synapse_type"],
                            ...
                            if self.gg.vs()['role'][coord_1] in ["NA", 2]:
                                if isinstance(interneurons[coord_1], keras.engine.sequential.Sequential):
                                    ...
                                    if apply_cross_conn:
                                        ...
                                        rel_s = str(self.gg.vs()['role'][coord_0]) + str(self.gg.vs()['role'][coord_1])
                                        #print("Trovato" + rel_s)
                                        interneurons[coord_1] = tf.keras.layers.Multiply(name=self.t_n(rel_s, gg_el["synapse_type"],
                            ...
                            if self.gg.vs()['role'][coord_1] in ["S", 1]:
                                if isinstance(sensors[coord_1], keras.engine.sequential.Sequential):
                                    ...
                                    if apply_cross_conn:
                                        ...
                                        rel_s = str(self.gg.vs()['role'][coord_0]) + str(self.gg.vs()['role'][coord_1])
                                        #print("Trovato" + rel_s)
                                        sensors_to[coord_1] = tf.keras.layers.Multiply(name=self.t_n(rel_s, gg_el["synapse_type"],
                            ...
                            if self.gg.vs()['role'][coord_1] in ["S", 0]:
                                if isinstance(sensors[coord_1], keras.engine.sequential.Sequential):
                                    ...
                                    if apply_cross_conn:
                                        ...
                                        rel_s = str(self.gg.vs()['role'][coord_0]) + str(self.gg.vs()['role'][coord_1])
                                        #print("Trovato" + rel_s)
                                        sensors[coord_1] = tf.keras.layers.Multiply(name=self.t_n(rel_s, gg_el["synapse_type"],
                                                                                             coord_0, coord_1))(
    ...
    def init_graph_inter_allocations(self, sensors, motors, interneurons,sensors_to, motors_to, interneurons_to, patch_pattern, activation_line):
        print("S to S")
        ...
        # input()
        return sensors, motors, interneurons
    def init_graph_allocations_resevoir(self, x, sensors, interneurons, motors,sensors_to, motors_to, interneurons_to, patch_pattern, activation_line):
        apply_cross_conn = True
        role_list = self.gg.vs()['role']
        i=0
        for el in role_list:
    ...
    def elegans_latent_graph_total(self, x, patch_pattern, activation_line=tf.nn.gelu):
        ...
        sensors, motors, interneurons = self.init_graph_allocations_resevoir(x,     sensors, motors, interneurons, sensors_to, motors_to, interneurons_to, patch_pattern, activation_line)
        sensors, motors, interneurons = self.init_graph_inter_allocations( sensors, motors, interneurons, sensors_to, motors_to, interneurons_to, patch_pattern, activation_line)
        ...
        l_x_mot_out = MultiHeadAttention(num_heads=motor_count, key_dim=32, attention_axes=(1, 2), trainable=False)
    ...
    def build6(self, m_lr=0.01, m_epochs=3, batch_size=32, ratio=1, node_number=280, patch_size=14):
        ...
        x = tf.keras.layers.Conv2D(16*factor, (3, 3), activation=act_fun,        padding='same',
                                   kernel_initializer=kernel_initializer, bias_initializer=bias_initializer, use_bias=True, trainable=False)(input_img)

        x = tf.keras.layers.MaxPooling2D((2, 2), padding='same')(x)

        x = tf.keras.layers.Conv2D(8*factor, (3, 3), activation=act_fun,         padding='same',
                                   kernel_initializer=kernel_initializer, bias_initializer=bias_initializer, use_bias=True, trainable=False)(x)
        ...
        xp = tf.keras.layers.Dense(patch_pattern, activation=act_fun,  #proviamo 1024
                                   kernel_initializer=kernel_initializer, bias_initializer=bias_initializer, use_bias=True, trainable=False)(xp)
    ...
    def build6_train(self, m_lr=0.01, m_epochs=3, batch_size=32, ratio=1, node_number=280, patch_size=28 ):
        ...
        model.fit(self.x_train, self.x_train, shuffle=True, batch_size=batch_size, epochs=m_epochs,
                            validation_data=(self.x_test, self.x_test),  callbacks=[csv_callback])


        return model

    def build6_test(self, model,  m_lr=0.01, m_epochs=3, batch_size=32, ratio=1, node_number=280, patch_size=28, subd="org", save_to="./ElegansMNIST_Autoencoder_dir/mnist_autoenc/"):
